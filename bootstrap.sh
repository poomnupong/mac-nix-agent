...
if [ "$local_user" != "$current_user" ] || [ "$local_host" != "$current_host" ]; then
    ...
    cat > "$local_nix" <<EOF
# Per-machine identity for flake.nix. Generated by bootstrap.sh.
# Gitignored — safe to edit by hand. Re-run bootstrap.sh to regenerate
